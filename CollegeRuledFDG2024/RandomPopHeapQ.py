...
def heaprandpop(h):
    range = len(h)
    i = random.randint(0, range-1) # Generate random in range index "i"
    elementToGrab = h[i]
    smallest = h[0]
    h[0] = elementToGrab # swap target and smallest element
    h[i] = smallest
    ele = h.pop() # Then pop it off
    if i < len(h): # and re-establish heap invariant
        _siftup(h, i)
        _siftdown(h, 0, i)
    return ele
